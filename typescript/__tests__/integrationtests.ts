...
describe("--Integration Tests--", () => {
  describe("--Method Tests--", () => {
    ...
    it(".pullFromTo receives all the data", async () => {
      const pullData = await client.pull("SELECT * FROM TESTJESTSTREAM;");
      const data = await client.pullFromTo(
        "TESTJESTSTREAM",
        "America/Los_Angeles",
        ["2022-01-01", "00", "00", "00"]
      );
    ...
    describe("--Materialized Streams and Tables--", () => {
      ...
      describe("--Materialized Streams Tests--", () => {
        ...
        beforeAll(async () => {
          ...
          testAsQueryId = await client.createStreamAs(
            "testAsStream",
            ["name", "age"],
            "newTestStream",
            {
      ...
      describe("--Materialized Tables Tests--", () => {
        beforeAll(async () => {
          await client.createTableAs(
            "testAsTable",
            "newTestStream",
            ["name", "LATEST_BY_OFFSET(age) AS recentAge"],
            { topic: "newTestTopic" },
            { WHERE: "age >= 21", GROUP_BY: "name" }
          );
        });
        afterAll(async () => {
          await client.ksql("DROP TABLE IF EXISTS testAsTable;");
  ...
  describe("--Health Tests--", () => {
    ...
    it(".inspectQueryStatus checks if a stream is created successfully", async () => {
      ...
      const create =
        await client.ksql(`CREATE STREAM IF NOT EXISTS ${streamName}
                      (name VARCHAR,
                      email varchar,
                      age INTEGER)
                   WITH (
