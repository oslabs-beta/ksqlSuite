...
/* const createTableTest = () => {
    client.createTable('AnotherTestTable', ['name ARCHAR PRIMARY KEY', 'email VARCHAR', 'age INTEGER'], 'users', 'json', 1);
};
...
//---------------------Test Create Stream As-------------------
/* let queryId;
const createStreamAsTest = async () => {
    await client.ksql('DROP STREAM IF EXISTS TestAsStream;')
    queryId = await client.createStreamAs('TestAsStream', ['latitude', 'longitude'], 'riderLocations', {
...
//---------------------Test Termination of Persistent Queries-------------------
/* const terminatePersistentQuery = async () => {
    try {
        const result = await client.ksql('TERMINATE ' + queryId + ';')
    } catch (error) {
        console.log(error);
    }
};

setTimeout(terminatePersistentQuery, 2000); */
